...
const CREATABLE_ROLES = ['PROVEEDOR', 'SUPERVISOR', 'DIRECTOR', 'ADMIN', 'VENDEDOR'];
...
const buttonSmallClass =
  'inline-flex items-center justify-center rounded-md border border-border bg-transparent px-3 py-1 text-[13px] font-medium text-text-h transition-all hover:border-accent-border hover:text-accent-500 active:scale-95 disabled:cursor-not-allowed disabled:opacity-60 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:ring-offset-1';
...
export default function AdminProveedores() {
  ...
  // PROVEEDOR/SUPERVISOR se restringen por proveedor; VENDEDOR se restringe por
  // su propio vendedor (sin proveedor); DIRECTOR/ADMIN ven todo siempre.
  const scopedUsers = useMemo(
    () => users.filter((u) => u.role === 'PROVEEDOR' || u.role === 'SUPERVISOR' || u.role === 'VENDEDOR'),
    [users],
  );
  ...
  const isVendedorSingleChoice = selectedUser?.role === 'VENDEDOR';
  ...
  const canSaveVendedores = isVendedorSingleChoice ? selectedVendedores.length === 1 : true;

  async function handleSaveVendedores() {
    if (!selectedUser || !canSaveVendedores) return;
  ...
  return (
    <div className="w-full flex-1 px-6 py-8 text-left sm:px-8">
      <PageHeader title="Administración" subtitle="Crea usuarios y administra qué proveedores y vendedores puede ver cada uno." />

      <section className="animate-fade-in-up mb-8 rounded-lg border border-border bg-surface-muted p-5" style={{ animationDelay: '60ms' }}>
            ...
            <tbody>
              {users.map((user) => (
                <Fragment key={user.id}>
                  <tr className="transition-colors hover:bg-accent-soft">
      ...
      <section className="animate-fade-in-up" style={{ animationDelay: '180ms' }}>
        ...
        <p className={`${resultTextClass} mb-4 max-w-3xl`}>
          ...
          además, vendedores (ambos se combinan: solo ve ventas que cumplan las dos condiciones). Vendedor:
          exactamente un vendedor asignado (obligatorio); solo ve los concursos donde ese vendedor está vinculado, y
          únicamente su propia fila. Sin ninguno asignado en una dimensión, no ve nada por esa dimensión.
        </p>

        {error && <p className={`${alertClass} mb-3`}>{error}</p>}

        <div className="mt-4 flex flex-wrap items-start gap-6">
          <div className="min-w-[220px] rounded-lg border border-border bg-surface p-4 shadow-soft">
            <h3 className="mb-2 text-base font-medium text-text-h">Usuarios</h3>
            {scopedUsers.length === 0 ? (
              <p className={resultTextClass}>No hay usuarios con rol Proveedor, Supervisor o Vendedor.</p>
            ) : (
                    ...
                    <button
                      ...
                      className={
                        String(user.id) === selectedUserId
                          ? 'flex w-full items-center justify-between gap-2 rounded-md border border-accent-border bg-accent-soft px-3 py-2 text-left text-accent-500 transition-all'
                          : 'flex w-full items-center justify-between gap-2 rounded-md border border-border bg-transparent px-3 py-2 text-left text-text-h transition-all hover:border-accent-border hover:translate-x-0.5'
                      }
                    >
                      {user.username}
                      <span className="rounded-full bg-surface-muted px-2 py-0.5 text-xs text-text">
                        ...
                        {user.role === 'VENDEDOR'
                          ? ` · ${user.vendedores.length} vend`
                          : ` · ${user.proveedores.length} prov${
          ...
          <div className="min-w-[260px] flex-1 rounded-lg border border-border bg-surface p-4 shadow-soft">
            {!selectedUser ? (
              <p className={resultTextClass}>Selecciona un usuario para ver y editar sus proveedores asignados.</p>
            ) : (
              <>
                {!isVendedorSingleChoice && (
                  <>
                ...
                {(selectedUser.role === 'SUPERVISOR' || isVendedorSingleChoice) && (
                  <>
                    <h3 className={`mb-2 text-base font-medium text-text-h${isVendedorSingleChoice ? '' : ' mt-6'}`}>
                      Vendedor{isVendedorSingleChoice ? '' : 'es'} de {selectedUser.username}
                    </h3>
                    {isVendedorSingleChoice && (
                      <p className={`${resultTextClass} mb-2`}>Elige exactamente un vendedor.</p>
                    )}
                    <div className="my-3 flex max-h-[400px] flex-col gap-1.5 overflow-y-auto rounded-lg border border-border px-4 py-2.5">
                      {vendedores.map((vendedor) => (
                        <label key={vendedor} className="flex flex-row items-center gap-2 text-sm text-text-h">
                          <input
                            type={isVendedorSingleChoice ? 'radio' : 'checkbox'}
                            name="vendedor-unico"
                    ...
                    {isVendedorSingleChoice && !canSaveVendedores && (
                      <p className={alertClass}>Debes elegir un vendedor.</p>
                    )}
                    {vendedorMessage && <p className={resultTextClass}>{vendedorMessage}</p>}
                    <button
                      type="button"
                      onClick={handleSaveVendedores}
                      disabled={savingVendedores || !canSaveVendedores}
                      className={`${buttonPrimaryClass} mt-2`}
                    >
                      {savingVendedores ? 'Guardando...' : 'Guardar vendedor(es)'}
                    </button>
                  </>
                )}
              </>
